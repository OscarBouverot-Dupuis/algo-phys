...
private:
    ...
    double time_quad_link(double d_f) {
        double b;
        if (sigma == 0) {
            b = dir_vec * d_f;
        }
    ...
    std::pair<double, int> time_quad() {
        ...
        if (sigma == 0) {
            ...
            std::tie(i, j) = coordinates(lift_label_w);
        }
    ...
    double time_cos(double t_quad) {
        ...
        double t_cos = std::floor(B2) - A + std::floor(A) + 1 / (Pi* 2) * std::acos(1 - 2 * (B2 - std::floor(B2)));
        return t_cos;
    }
    ...
    void metropolis_J_move() {
        if (head_label == tail_label && UniformDouble(gen) < 0.5) {
            ...
            //refresh w lifting variables
            dir_vec_w = 2 * UniformUnit(gen) - 1;
            auto[ih, jh] = coordinates(head_label);
            all_lift_label_w = { index(ih, jh), index(ih + 1,jh), index(ih, jh + 1), index(ih + 1,jh + 1) }; //change for more smoothness
            lift_label_w = all_lift_label_w[UniformChoice(gen)];
        }
        else {
            auto [ih, jh] = coordinates(head_label);
            ...
            if (UniformDouble(gen) < r * std::exp(-dE_b(i, j, dx, dt))) {
                ...
                //refresh w lifting variables
                dir_vec_w = 2 * UniformUnit(gen) - 1;
                std::tie(ih, jh) = coordinates(head_label);
                all_lift_label_w = { index(ih, jh), index(ih + 1,jh), index(ih, jh + 1), index(ih + 1,jh + 1) }; //change for more smoothness
                lift_label_w = all_lift_label_w[UniformChoice(gen)];
            }
        }
...
public:
    ...
    int dir_vec_w = 1;
    int lift_label_w = 0;
    std::vector<int> all_lift_label_w;
    int sigma = 0;
    int head_label = 0;
    int tail_label = 0;
    int64_t algo_time = 0;
    ...
    PhysSystem(int L, int B, double K, double g, double mu, double lambda_r, double lambda_w)
        ...
        lambda_r{ lambda_r },
        lambda_w{ lambda_w },
        gen(rd()), UniformDouble(0.0, 1.0), UniformUnit(0.0, 1.0), UniformLabel(0.0, L* B - 1), Gaussian(0, 1), UniformChoice(0.0, 3)
    {}
    ...
    void update_lift_var(std::string& event_type, int partner_label) {
        if (sigma == 0) {
            ...
            if (event_type == "quad") {
                if (std::find(all_lift_label_w.begin(), all_lift_label_w.end(), partner_label) != all_lift_label_w.end()) {//partner_label is an allowed lift_label_w
                    lift_label_w = partner_label;
                }
                else {
                    dir_vec_w *= -1;
                }
    ...
    void do_lift(double distance) {
        ...
        if (sigma == 0) {
            ...
            f[lift_label_w] = f[lift_label_w] + distance * dir_vec_w;
        }
